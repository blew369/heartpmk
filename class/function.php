...
class DB_con{
    ...
    public function signin($username, $password){
        $stmt = $this->dbcon->prepare("SELECT members.username, members.memberid, members.hn, members.prename, CONCAT_WS(' ',members.patientname,members.patientlname) AS fullname, members.weight, drugtype.drugname, members.drug, swellsymp.swellname, tiredsymp.tiredname
                                        FROM members
                                        LEFT JOIN swellsymp ON swellsymp.swellid = members.swellid
                                        LEFT JOIN tiredsymp ON tiredsymp.tiredid = members.tiredid
                                        LEFT JOIN drugtype ON drugtype.drugid= members.drugid  
    ...
    public function GetSumUp($reqid){
        $stmt = $this->dbcon->prepare("SELECT heartrequest.hn, members.prename, members.patientname, members.patientlname, swellsymp.swellname, tiredsymp.tiredname,
                                            members.weight, members.swellid, members.tiredid, heartrequest.drug, heartrequest.reid, CONCAT_WS('', heartrequest.weight) As weightreq,
                                            CONCAT_WS('', heartrequest.swellid) As swellidreq,  CONCAT_WS('', heartrequest.tiredid) As tiredidreq, heartrequest.other, 
                                            drugtype.drugname, heartrequest.drugsum, heartrequest.weightbf
                                        FROM heartrequest
                                        INNER JOIN swellsymp ON heartrequest.swellid = swellsymp.swellid
                                        INNER JOIN tiredsymp ON heartrequest.tiredid = tiredsymp.tiredid
                                        INNER JOIN members ON heartrequest.hn = members.hn
    ...
    public function getRequest($hn){
        $query=$this->dbcon->prepare("SELECT heartrequest.hn, members.prename, CONCAT_WS(' ',members.patientname, members.patientlname) AS fullname, swellsymp.swellname, tiredsymp.tiredname,
                        members.weight, members.swellid, members.tiredid, heartrequest.drug, heartrequest.reid, CONCAT_WS('', heartrequest.weight) As weightreq,
                    ...
                    INNER JOIN members ON heartrequest.hn = members.hn
                    INNER JOIN drugtype ON heartrequest.drugid = drugtype.drugid
                    WHERE heartrequest.hn= ? AND heartrequest.status=1 
                    ORDER BY heartrequest.reid ASC");
        $query->bind_param("s", $hn);
        $query->execute();
