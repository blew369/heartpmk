		...
		<div id="page-container" class="fade page-sidebar-fixed page-header-fixed">
			...
			<div id="content" class="content">
				...
				<ol class="breadcrumb pull-right">
					...
					<li class="breadcrumb-item active">Drug Type Managed Tables</li>
				</ol>
				<!-- end breadcrumb -->
				<!-- begin page-header -->
				<h1 class="page-header">จัดการชนิดยา</h1>
				<!-- end page-header -->
				...
				<div class="panel panel-inverse">
					...
					<div class="panel-heading bg-inverse">
						...
						<h4 class="panel-title">ข้อมูลชนิดยา</h4>
					</div>
					<!-- end panel-heading -->
					<!-- begin panel-body -->
					<div class="panel-body" id="patient">
						<div class="mb-3" align="left">
							<button onclick="document.getElementById('id').value = ''" type="button" name="add" id="add" data-toggle="modal" class="btn btn-green" data-target="#addModal"><i class="fas fa-plus-circle"></i> เพิ่มชนิดยา</button>
						</div>
						<table id="data-table-default" class="table table-striped table-bordered mt-5">
							<thead>
								<tr>
									<th width="10%" class="text-nowrap">ระดับอาการบวม</th>
                                    <th width="10%" class="text-nowrap">เลขอ้างชนิดยา</th>
							...
							<tbody>
								<?php
								$result = $conDB->Fetchdrugtyep();
								while ($row = $result->fetch_assoc()) {
								?>
									<tr class="odd gradeX">
										<td width="10%"><?php echo $row['drugname']; ?></td>
                                        <td width="10%"><?php echo $row['drugid']; ?></td>
								...
								<div class="modal-body">
									<form method="post" id="insert_form">
										<div class="form-group col-mb-3">
											...
											<label for="triedname" class="form-label mt-2">ชื่อชนิดยา</label>
											<input type="text" class="form-control" name="drugname" id="drugname" required>
										</div>
                                        <div class="form-group col-mb-3">
											<label for="tiredid" class="form-label mt-2">เลขอ้างชนิดยา</label>
											<input type="text" class="form-control" name="drugid" id="drugid" required>
										</div>
	...
	<![endif]-->
			...
			<script>
				$(document).ready(function() {
                    ...
                    $('#addModal').modal('show');
                    $('#insert_form')[0].reset();
                    $('.modal-title').text("เพิ่มชนิดยา");
                    $('#insert').val("INSERT");
                });
					$('#insert_form').on('submit', function(e) {
						e.preventDefault();
						$.ajax({
							url: "../class/masdrugtypeInsert.php",
							method: "post",
					...
					$('.delete_data').click(function() {
						var id = $(this).attr("id");
						var status = confirm("คุณต้องการลบชนิดยานี้ใช่หรือไม่?");
						if (status) {
							$.ajax({
								url: "../class/masdrugtypeDelete.php",
								method: "post",
								data: {
									id: id
								},
					...
					$('.edit_data').click(function() {
						var id = $(this).attr("id");
						$.ajax({
							url: "../class/masdrugtypeFetch.php",
							method: "post",
							data: {
								id: id
							},
