		...
		<div id="page-container" class="fade page-sidebar-fixed page-header-fixed">
			...
			<div id="content" class="content">
				...
				<ol class="breadcrumb pull-right">
					...
					<li class="breadcrumb-item active">Swell Managed Tables</li>
				</ol>
				<!-- end breadcrumb -->
				<!-- begin page-header -->
				<h1 class="page-header">จัดการระดับอาการบวม</h1>
				<!-- end page-header -->
				...
				<div class="panel panel-inverse">
					...
					<div class="panel-heading bg-inverse">
						...
						<h4 class="panel-title">ข้อมูลระดับอาการบวม</h4>
					</div>
					<!-- end panel-heading -->
					<!-- begin panel-body -->
					<div class="panel-body" id="patient">
						<div class="mb-3" align="left">
							<button onclick="document.getElementById('swid').value = ''" type="button" name="add" id="add" data-toggle="modal" class="btn btn-green" data-target="#addModal"><i class="fas fa-plus-circle"></i> เพิ่มระดับอาการบวม</button>
						</div>
						<table id="data-table-default" class="table table-striped table-bordered mt-5">
							<thead>
								<tr>
									<th width="10%" class="text-nowrap">ระดับอาการบวม</th>
							...
							<tbody>
								<?php
								$result = $conDB->FetchAllSwell();
								while ($row = $result->fetch_assoc()) {
								?>
									<tr class="odd gradeX">
										<td width="10%"><?php echo $row['swellname']; ?></td>
										<td width="10%"><?php echo $row['swellid']; ?></td>
										<td width="1%"><button type="button" name="edit" value="edit" class="btn btn-warning btn-sm edit_data" id="<?php echo $row['swid']; ?>"><i class="fas fa-edit"></i> แก้ไข</button></td>
										<td width="1%"><button type="button" name="delete" value="delete" class="btn btn-danger btn-sm delete_data" id="<?php echo $row['swid']; ?>"><i class="fas fa-window-close"></i> ลบ</button></td>
									</tr>
								<?php
								...
								<div class="modal-body">
									<form method="post" id="insert_form">
										<div class="form-group col-mb-3">
											...
											<input type="text" class="form-control" name="swellname" id="swellname" required>
										</div>
                                        <div class="form-group col-mb-3">
											<label for="tiredid" class="form-label mt-2">เลขอ้างอิงอาการ</label>
											<input type="text" class="form-control" name="swellid" id="swellid" required>
										</div>
			...
			<script>
				$(document).ready(function() {
					...
					$('.delete_data').click(function() {
						...
						var status = confirm("คุณต้องการลบอาการบวมนี้ใช่หรือไม่?");
						if (status) {
							$.ajax({
								url: "../class/swellDelete.php",
								method: "post",
								data: {
									id: swid
								},
					...
					$('.edit_data').click(function() {
						...
						$.ajax({
							...
							success: function(data) {
								$('#swid').val(data.swid);
								$('#swellname').val(data.swellname);
								$('#swellid').val(data.swellid);
								$('#insert').val("Update");
                                $('.modal-title').text("แก้ไขระดับอาการบวม");
								$('#addModal').modal('show');
							}
						});
